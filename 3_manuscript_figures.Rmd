...
glmmTMB(indicator1_mean ~ species_range + (1|country_assessment), family = "betabinomial", data = data)
```
...
glmmTMB(indicator2_mean ~ species_range + (1|country_assessment), family = "betabinomial", data = data)
...
p1<-df %>%
    ggplot(aes(x=myaxis, y=indicator1_mean, fill=taxonomic_group_simplified, color=taxonomic_group_simplified)) +
      ...
      coord_flip() +
      scale_x_discrete(limits=rev) +
      scale_fill_manual(values= c(rep("#9d9bb8", 5), # for animals
                                  rep("#7fb4a6", 4), # for platns
                                  rep("#b6a080", 1)), # for fungi and others
                       breaks=c(levels(df$taxonomic_group_simplified))) +
      scale_color_manual(values= c(rep("#9d9bb8", 5), # for animals
                                  rep("#7fb4a6", 4), # for platns
                                  rep("#b6a080", 1)), # for fungi and others
                       breaks=c(levels(df$taxonomic_group_simplified))) +
      theme_light() +
...
x  <- indicators_averaged_one %>%
               ...
               summarize(n.Ne_ind=n(),
                         mean.Ne_ind=mean(indicator1_mean),
                         n.below.75=sum(indicator1_mean<0.75),
                         n.below.50=sum(indicator1_mean<0.50),
                         n.below.25=sum(indicator1_mean<0.25),
                         per.below.25=n.below.25/n.Ne_ind*100,
...
total_counts <- summarise(x,
                          ...
                          n.Ne_ind = sum(n.Ne_ind),
                          mean.Ne_ind = mean(mean.Ne_ind),
...
x  <- indicators_averaged_one %>%
               ...
               group_by(taxonomic_group_simplified) %>% 
               summarize(n.PM_ind=n(),
                         mean.PM_ind=mean(indicator2_mean),
                         n.below.75=sum(indicator2_mean<0.75),
                         n.below.50=sum(indicator2_mean<0.50),
                         n.below.25=sum(indicator2_mean<0.25),
                         per.below.25=n.below.25/n.PM_ind*100,
...
total_counts <- summarise(x,
                          taxonomic_group_simplified = "ALL",
                          n.PM_ind = sum(n.PM_ind),
                          mean.PM_ind = mean(mean.PM_ind),
                          n.below.75 = sum(n.below.75),
                          n.below.50 = sum(n.below.50),
                          n.below.25 = sum(n.below.25),
                          per.below.25 = n.below.25 / n.PM_ind * 100,
...
### Violin plots but grouping animals, plants, others
...
Plot for Ne > 500 indicator:
```{r}
p3<-df %>%
  ggplot(aes(x=taxonomic_group_3, y=indicator1_mean, fill=taxonomic_group_3, color=taxonomic_group_3)) +
  geom_violin(width=1, linewidth = 0.2)  +
  geom_jitter(size=.7, width = 0.1, color="darkred") +
  xlab("") + ylab("Proportion of populations with Ne>500") +
  coord_flip() +
  scale_x_discrete(limits=rev) +
  scale_fill_manual(values= c("#9d9bb8", "#7fb4a6", "#b6a080"),
                    breaks = c("animals", "plants", "others")) +
  scale_color_manual(values= c("#9d9bb8", "#7fb4a6", "#b6a080"),
                     breaks = c("animals", "plants", "others")) +
  theme_light() +
  theme(panel.border = element_blank(), legend.position="none",
        text= element_text(size=15))
...
x  <- df %>%
               ...
               summarize(n.ind=n(),
                         mean.ind=mean(indicator1_mean),
                         n.below.75=sum(indicator1_mean<0.75),
                         n.below.50=sum(indicator1_mean<0.50),
                         n.below.25=sum(indicator1_mean<0.25),
                         per.below.25=n.below.25/n.ind*100,
                         per.below.50=n.below.50/n.ind*100)
...
Plot Proportion of mantained indicator:

```{r}
p4<-df %>%
  ggplot(aes(x=taxonomic_group_3, y=indicator2_mean, fill=taxonomic_group_3, color=taxonomic_group_3)) +
  geom_violin(width=1, linewidth = 0.2)  +
  geom_jitter(size=.7, width = 0.1, color="darkred") +
  xlab("") + ylab("Proportion of maintained populations") +
  coord_flip() +
...
x  <- df %>%
               ...
               group_by(taxonomic_group_3) %>% 
               summarize(n.ind=n(),
                         mean.ind=mean(indicator2_mean),
                         n.below.75=sum(indicator2_mean<0.75),
                         n.below.50=sum(indicator2_mean<0.50),
                         n.below.25=sum(indicator2_mean<0.25),
                         per.below.25=n.below.25/n.ind*100,
                         per.below.50=n.below.50/n.ind*100,
                         per.below.50=n.below.75/n.ind*100)

kable(x, digits=2)
```


### Density plots faceted by taxonomic group:

Ne > 500 indicator:

```{r}
density_plot <- df %>%
  ggplot(aes(x = indicator1_mean, fill = taxonomic_group_simplified, color = taxonomic_group_simplified)) +
  geom_density(alpha = 0.7) +
  facet_wrap(~ taxonomic_group_simplified, scales = "free_x", ncol=5) +
  labs(x = "Proportion of populations with Ne>500", y = "Density") +
  scale_fill_manual(
    values = c(rep("#9d9bb8", 5), # for animals
               rep("#7fb4a6", 4), # for plants
               rep("#b6a080", 1)), # for fungi and others
    breaks = levels(df$taxonomic_group_simplified)
  ) +
  scale_color_manual(
    values = c(rep("#9d9bb8", 5), # for animals
               rep("#7fb4a6", 4), # for plants
...
Proportion of maintained populations indicator:

```{r}
# Create density plots for indicator2_mean
density_plot <- df %>%
  ggplot(aes(x = indicator2_mean, fill = taxonomic_group_simplified, color = taxonomic_group_simplified)) +
  geom_density(alpha = 0.7) +
  facet_wrap(~ taxonomic_group_simplified, scales = "free_x", ncol=5) +
  labs(x = "Proportion of maintained populations", y = "Density") +
  scale_fill_manual(
    values = c(rep("#9d9bb8", 5), # for animals
               rep("#7fb4a6", 4), # for plants
               rep("#b6a080", 1)), # for fungi and others
...
### Density plots in a single plot with alpha:

Ne > 500 indicator:

```{r}
# Create a density plot for indicator1_mean without facets
density_plot <- df %>%
  ggplot(aes(x = indicator1_mean, fill = taxonomic_group_simplified, color = taxonomic_group_simplified)) +
  geom_density(alpha = 0.3, size = 1) + # Increase transparency and line size
  labs(x = "Proportion of populations with Ne>500", y = "Density") +
  scale_fill_viridis_d(option = "D", direction = -1) + # Use viridis color palette for fill
  scale_color_viridis_d(option = "D", direction = -1) + # Use viridis color palette for color
  theme_light() +
...
Proportion of mantaiend populations indicator:

```{r}
# Create a density plot for indicator2_mean without facets
density_plot <- df %>%
  ggplot(aes(x = indicator2_mean, fill = taxonomic_group_simplified, color = taxonomic_group_simplified)) +
  geom_density(alpha = 0.3, size = 1) + # Increase transparency and line size
  labs(x = "Proportion of maintained populations", y = "Density") +
...
### Histogram in a single plot by animals, plants and others:
...
histogram_plot <- df %>%
  ...
  geom_histogram(alpha = 0.7, bins = 20) + # Adjust the number of bins as needed
  labs(x = "Proportion of populations with Ne>500", y = "Frequency") +
  scale_fill_manual(
    values = c("#9d9bb8", "#7fb4a6", "#b6a080"), # Custom colors for animals, plants, and others
    breaks = c("animals", "plants", "others"),
...
Proportion of maintained populations indicator
```{r}
# Create a histogram plot instead of a density plot
histogram_plot <- df %>%
  ggplot(aes(x = indicator2_mean, fill = taxonomic_group_3)) +
  geom_histogram(alpha = 0.7, bins = 25) + # Adjust the number of bins as needed
