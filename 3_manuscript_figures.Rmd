...
### add them
## indicators
indicators_full$defined_populations_nicenames<-as.factor(indicators_full$defined_populations_simplified)
# add new nice names
levels(indicators_full$defined_populations_nicenames)<-nice_names_or
# change to desired order
indicators_full$defined_populations_nicenames<-factor(indicators_full$defined_populations_nicenames, 
                                                levels=nice_names)
...
glm(indicator2 ~ N_maintained_pops, family = “quasibinomial”). 
...
We run a similar model than above, but limited to relationship of interest:
glm(indicator1 ~ N_maintained_pops, family = “quasibinomial”). 
...
## Plot 3 panels
plot_grid(p1, p2.1, p3.1, ncol=3, rel_widths = c(1.7,1,1), align = "h", labels=c("a)", "b)", "c)"))  
...
```{r}
# plot
plot_grid(p4, p5, ncol=2, align = "h", labels=c("a)", "b)"))  
...
Plot Indicator 1 by type of range in the entire dataset. Filtering "unknown" or NA in species range:

```{r indicator1 by range}
# get sample size by desired category
sample_size <- indicators_averaged_one  %>%
                    filter(!is.na(indicator1_mean)) %>% 
                    filter(species_range =="wide_ranging" | species_range =="restricted") %>% 
...
p1
```
Plot Indicator 1 by country and type of range
...
```
Test the effect of range type on Ne>500 indicator. Does the indicator vary between wide randing vs restricted distribution species? 
...
Plot Indicator 2 by type of range in the entire dataset. Filtering "unknown" or NA in species range:
...
Plot Indicator 2 by country and type of range
...
Test the effect of range type on the proportion of maitained populations. Does the indicator vary between wide randing vs restricted distribution species? 
