...
Get population data for those species assessed using the text template instead of Kobo (PENDING):
```{r, eval=FALSE}
ind1_data_templates<- get_attached_data(root_dir = "attachments_2023_08_16_14_07_30", # the directory after unziping the kobo attachments download  
...
## Effect of method used to define populations on number of populations and Proportion of maintained populations (indicator) 
...
### Does the proportion of maintained populations (indicator2) vary with method used?
...
```{r}
m2<-glm(data_for_model$indicator2 ~ data_for_model$defined_populations_simplified + 
            data_for_model$country_assessment, family ="quasibinomial")
```
...
### (c) Is there a relationship between number of maintained populations and Indicator2, overall, and/or with some methods?
...
### Single plot 3 panels for the effect of method to define populations on Proportion of maintained populations (indicator) and number of pops
