...
# Extract Proportion of mantained populations (indicator) data from kobo output, show most relevant columns
...
### Proportion of mantained populations (indicator) (proportion of populations within species which are maintained)

Proportion of mantained populations (indicator) is the he proportion of populations within species which are maintained. This can be estimated based on the `n_extant_populations` and `n_extint_populations`, as follows:
...
```
We can add this new column to the metadata and Proportion of mantained populations (indicator) data:
...
For Proportion of mantained populations (indicator):
```{r}
indicators_averaged %>%
  filter(taxon == "Ambystoma cingulatum") %>%
...
We have NA in Proportion of mantained populations (indicator) because in some cases the number of extinct populations is unknown, therefore the operation cannot be computed. 
...
## Effect of method used to define populations on number of populations and Proportion of mantained populations (indicator) 
...
### Proportion of mantained populations (indicator) by method
...
## plot for Proportion of mantained populations (indicator)
p2<- df %>%
  filter(n_extant_populations<500) %>%
  ggplot(aes(x=myaxis, y=indicator2, color=defined_populations_simplified,    
                                     fill=defined_populations_simplified)) +
          geom_boxplot() + xlab("") + ylab("Proportion of mantained populations (indicator)") +
          geom_jitter(size=.4, width = 0.1, color="black") +
  coord_flip() +
  theme_light() +
  theme(panel.border = element_blank(), legend.position="none",
...
p3<- ind2_data %>%
  ...
  # plot
    ...
    ylab("Proportion of mantained populations (indicator)") +
...
### Single plot 3 panels for the effect of method to define populations on Proportion of mantained populations (indicator) and number of pops

Plot in three panels. 
```{r, out.height="380px", out.width="1130px"}
## plot for Proportion of mantained populations (indicator) without axis labels
p2<-ind2_data %>% 
  filter(n_extant_populations<500) %>%
...
## Values of indicator 1 and Proportion of mantained populations (indicator) for multiassessed species
...
Now check how Proportion of mantained populations (indicator) changes across the multiassessments.
