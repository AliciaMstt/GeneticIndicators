...
df$myaxis<-factor(df$myaxis, 
                  #grep is used below to get the sample size, which may change depending on the data
                    levels=c(grep("amphibian", unique(df$myaxis), value = TRUE), 
                             ...
                             grep("bryophyte", unique(df$myaxis), value = TRUE),
                             ...
                             grep("fungus", unique(df$myaxis), value = TRUE),
                             grep("other" , unique(df$myaxis), value = TRUE)))

df$taxonomic_group<-factor(df$taxonomic_group, 
                           levels=c("amphibian", "bird" , "fish" , "invertebrate", "mammal",
                                     "angiosperm",  "bryophyte",  "gymnosperm",  "reptile", "pteridophytes",
                                     "fungus", "other"))

        
# plot
p1<-df %>%
    ggplot(aes(x=myaxis, y=indicator1_mean, fill=taxonomic_group, color=taxonomic_group)) +
      ...
      scale_fill_manual(values= c(rep("#9d9bb8", 5), # for animals
                                  rep("#7fb4a6", 5), # for platns
                                  rep("#b6a080", 2)), # for fungi and others
                       breaks=c(levels(df$taxonomic_group))) +
...
Table of number or percentage of taxa below 0.25, 0.50 and 0.75:
...
# plot
p2<-df %>%
    ggplot(aes(x=myaxis, y=indicator2_mean, fill=taxonomic_group, color=taxonomic_group)) +
...
### Single figure 2 panels for both indicators by taxonomic group
```{r}
plot_grid(p1, p2, ncol=1, align = "v", labels=c("a)", "b)"))
```
