...
### Sampling size and plot of Ne values by Ne origin and method to define populations
...
p1<-df %>%
    ggplot(aes(x=myaxis, y=indicator1_mean, fill=taxonomic_group_simplified, color=taxonomic_group_simplified)) +
      ...
      coord_flip() +
      scale_x_discrete(limits=rev) +
      scale_fill_manual(values= c(rep("#6d6b8c", 5), # for animals
                                  rep("#548b7c", 4), # for platns
                                  rep("#8c7a55", 1)), # for fungi and others
                       breaks=c(levels(df$taxonomic_group_simplified))) +
      scale_color_manual(values= c(rep("#6d6b8c", 5), # for animals
                                  rep("#548b7c", 4), # for platns
                                  rep("#8c7a55", 1)), # for fungi and others
                       breaks=c(levels(df$taxonomic_group_simplified))) +
      theme_light() +
...
## Simplified figures for graphical summary and policy brief
...
### Violin plot for Ne > 500 indicator animals, plants, others
```{r}
p<-indicators_averaged_one %>%
                ...
                coord_flip() +
                scale_x_discrete(limits=rev) +
                scale_fill_manual(values= c("#6d6b8c", "#548b7c", "#8c7a55"),
                                  breaks = c("animals", "plants", "others")) +
                scale_color_manual(values= c("#6d6b8c", "#548b7c", "#8c7a55"),
                                   breaks = c("animals", "plants", "others")) +
                theme_light() +
                theme(panel.border = element_blank(), legend.position="none",
                      text= element_text(size=15))
...
hist_p <- indicators_averaged_one %>%
                  ...
                  labs(x = "roportion of populations with Ne>500", y = "Frequency") +
                  scale_fill_manual(
                    values = c("#6d6b8c", "#548b7c", "#8c7a55"), # Custom colors for animals, plants, and others
                    breaks = c("animals", "plants", "others"),
...
### Histogram for Proportion Mantained populations for animal, plants, others
