...
## Main Figure: Data availability to estimate Ne indciator (origion of data to estimate Ne) and PM indicator (missing data on pop extinction):
...
Run model asking: Does method still influence number of populations if we exclude range type from the model:
...
Do different methods give different numbers of populations, when controlling for range type? Does range type influence the number of populations, when controlling for method ?
...
Does range type still influence the number of populations even if we exclude method from the model?
```{r}
m1.2<-glmer(n_extant_populations ~ species_range + (1|country_assessment), family = "poisson", data = data_for_model)
...
summary(m2)
```
Method influences the number of populations, and method influences indicator2. Is the relationship between method and indicator2 an indirect result of the effect of number of populations on indicator2? I.e. does method still influence indicator2 if we control for number of populations? [
```{r}
m2.1<-glmmTMB(indicator2 ~ defined_populations_simplified + n_extant_populations + (1|country_assessment), family = "ordbeta", data = data_for_model)
...
summary(m3)
```
Method influences the number of populations, and method influences indicator1. Is the relationship between method and indicator1 an indirect result of the effect of number of populations on indicator1? I.e. does method still influence indicator1 if we control for number of populations?

```{r}
m3.1 <- glmmTMB(indicator1 ~ defined_populations_simplified + n_extant_populations + (1|country_assessment), family = "ordbeta", data = data_for_model)
...
We run a similar model than above, but limited to relationship of interest. Does number of populations alone affect indicator2 (i.e. not controlling for method)?:
...
```{r}
# run model
m4.2glm <- glm(ind2_single_methods$indicator2 ~ ind2_single_methods$n_extant_populations + 
...
```{r}
m4.2<-glmmTMB(indicator2 ~ n_extant_populations + defined_populations_simplified +        n_extant_populations*defined_populations_simplified + (1|country_assessment), family = "ordbeta", data = ind2_single_methods)
...
We run a similar model than above, but limited to relationship of interest. We are asking: Does number of populations alone influence indicator 1 (i.e. not controlling for method)? 

```{r}
m5.1<-glmmTMB(indicator1 ~ n_extant_populations + (1|country_assessment), family = "ordbeta", data= data_for_model)
```
...
```{r, }
# run model
m5.2 <- glmmTMB(indicator1 ~ n_extant_populations + defined_populations_simplified +        n_extant_populations*defined_populations_simplified + (1|country_assessment), family = "ordbeta", data = ind2_single_methods)
```
...
### Main Figure: Single plot 3 panels box plots for the effect of method on: number of populations, proportion of maintained populations (indicator 2) and Proportion of populations with Ne>500 (indicator 1).
...
### Main Figure: Violin plots indicatrors by of range type coloring points to show genetic clusters
