...
levels(metadata$defined_populations_nicenames)<-nice_names
```
...
ggplot(metadata, aes(x=taxonomic_group, fill=popsize_data)) + 
  ...
  scale_fill_manual(values=c("#1f77b4", "grey80", "#2ca02c"),
                    ...
                    labels=c("Species or subspecies level", "Insufficient data", "Population level")) +
...
## Effect of method used to define populations on number of populations and indicator 2 (alluvial plot)
...
ind2_data %>% 
  ...
  ggplot(aes(x=defined_populations_nicenames, y=n_extant_populations, color=defined_populations_nicenames)) +
          geom_boxplot() +
