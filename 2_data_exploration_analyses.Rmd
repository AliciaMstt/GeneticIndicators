...
metadata_unique<-metadata[!duplicated(cbind(metadata$taxon, metadata$country_assessment)), ]
...
```{r}
metadata_unique %>% # we use the _unique dataset so that multiassesed records are counted only once
        filter(multiassessment=="multiassessment") %>%
...
Note: The following plots in this section consider only one record of the taxa that were assessed more than once. That is a total of `r nrow(metadata_unique)` taxa.
...
PENDIENTE ¿QUÉ HACER CON MULTIASSESMENTS?
