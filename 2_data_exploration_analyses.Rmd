...
# transform data to how ggsankey wants it
df <- metadata %>%
  make_long(country_assessment, defined_populations_simplified)
...
ggplot(df, aes(x = x,
  ...
  # colour by country
  scale_fill_manual(values=c(scales::hue_pal()(length(unique(metadata$country_assessment))),  # a nice  color for each of the n countries
                             rep("darkgrey", length(unique(metadata$defined_populations_simplified)))), # grey for the n taxonomic groups
                    breaks=c(unique(metadata$country_assessment), 
                             unique(metadata$defined_populations_simplified))) +
  theme_sankey(base_size = 10) +
  xlab("")
```
...
p1<- x %>% 
  ...
  scale_color_manual(values=scales::hue_pal()(length(levels(as.factor(ind2_data_firstmulti$defined_populations_simplified)))),
