...
To simplify from those many combinations of methods to define populations, we will have a "simplified version" where we keep only each method alone, plus "method_and_others" if it occurs in combination with any other, with the exception of the combination "genetic_cluster geographic_boundaries" because they are two more relevant both methodologically and in abundance. We also create a "ultra simplified version" where methods with few records alone or with "_and_others" are clustered together in "method_alone_orwith_others".
...
ind2_data<- ind2_data %>% 
  mutate(defined_populations_simplified = case_when(
          defined_populations=="genetic_clusters geographic_boundaries" ~ "genetic_clusters_and_geographic_boundaries",
          grepl("genetic_clusters ", defined_populations) ~ "genetic_clusters_and_others",
          grepl("geographic_boundaries ", defined_populations) ~ "geographic_boundaries_and_others",
          grepl("eco_biogeo_proxies ", defined_populations) ~ "eco_biogeo_proxies_and_others",
          grepl("adaptive_traits ", defined_populations) ~ "adaptive_traits_and_others",
          grepl("management_units ", defined_populations) ~ "management_units_and_others",
          grepl("other ", defined_populations) ~ "other_and_others",
          TRUE ~ defined_populations)) %>% 
  mutate(defined_populations_ultrasimplified = case_when(
          grepl("adaptive_traits", defined_populations_simplified) ~ "adaptive_traits_alone_orwith_others",
          grepl("eco_biogeo_proxies", defined_populations_simplified) ~ "eco_biogeo_proxies_alone_orwith_others",
          grepl("management_units", defined_populations_simplified) ~ "management_units_alone_orwith_others",
          TRUE ~ defined_populations_simplified))
...
```{r}
ind2_data %>% 
       select(defined_populations, defined_populations_simplified, defined_populations_ultrasimplified) %>% 
       filter(!duplicated(defined_populations))
```
The problem with the former is that only the first method of the list is retained, so for instance in the following example "adaptive_traits" is clustered as "others" and not counted. To avoid this we need to think in a different way of counting the methods. Maybe with a 1/0 matrix with a column for each method. 

```{r}
ind2_data %>% 
       select(defined_populations, defined_populations_simplified, defined_populations_ultrasimplified) %>% 
       filter(defined_populations=="eco_biogeo_proxies adaptive_traits")
```
...
Check n for ultrasimplified methods:

```{r}
table(ind2_data$defined_populations_ultrasimplified)
```
...
```{r}
# reformat data
foralluvial<-metadata %>% group_by(country_assessment, defined_populations_ultrasimplified, taxonomic_group) %>%
             summarise(n=n())

## plot
# define colors
my_cols<- gg_color_hue(length(unique(foralluvial$country_assessment)))

# we need a vector of colors by country for each row of the dataset, so:
countries<-as.factor(foralluvial$country_assessment)
levels(countries)<-my_cols
countries<-as.vector(countries)
head(countries)
...
# transform data to how ggsankey wants it
df <- metadata %>%
  make_long(country_assessment, defined_populations_ultrasimplified)
...
ggplot(df, aes(x = x,
  ...
  # colour by country
  scale_fill_manual(values=c(scales::hue_pal()(length(unique(metadata$country_assessment))),  # a nice  color for each of the n countries
                             rep("darkgrey", length(unique(metadata$defined_populations_ultrasimplified)))), # grey for the n taxonomic groups
                    breaks=c(unique(metadata$country_assessment), 
                             unique(metadata$defined_populations_ultrasimplified))) +
  theme_sankey(base_size = 10) +
  xlab("")
```
...
Same with simplified method categories:

```{r, out.width=900, out.height=500}
ind2_data %>% 
  filter(n_extant_populations<200) %>%
  ggplot(aes(x=defined_populations_simplified, y=n_extant_populations)) +
          geom_boxplot() +
  coord_flip() +
  theme_bw() +
  theme(panel.border = element_blank())
...
Same with ultrasimplified method categories:

```{r, out.width=900, out.height=500}
ind2_data %>% 
  filter(n_extant_populations<200) %>%
  ggplot(aes(x=defined_populations_ultrasimplified, y=n_extant_populations)) +
          geom_boxplot() +
  coord_flip() +
  theme_bw() +
  theme(panel.border = element_blank())
...
sample_size <- ind2_data_firstmulti %>%
                    filter(!is.na(indicator2)) %>% 
                    group_by(defined_populations_ultrasimplified) %>% summarize(num=n())

# plot
highlight12<-ind2_data_firstmulti %>% 
  # add sampling size 
  left_join(sample_size) %>%
  mutate(myaxis = paste0(defined_populations_ultrasimplified, " (n= ", num, ")")) %>%

  # plot
  ggplot(aes(x=myaxis, y=indicator2, fill=defined_populations_ultrasimplified)) +
  geom_violin(width= 1, linewidth = 0)  +
  geom_jitter(size=.5, width = 0.1) +
  xlab("") +
  coord_flip() +
...
highlight13<-ind2_data_firstmulti %>% 
  ...
  # plot
  ggplot(aes(x=myaxis, y=indicator2, color=defined_populations_ultrasimplified)) +
  geom_boxplot()  +
...
highlight14<-ind2_data_firstmulti %>% 
  ...
  # plot
  ggplot(aes(x=defined_populations_ultrasimplified, y=indicator2, color=defined_populations_ultrasimplified)) +
  geom_boxplot()  +
  geom_jitter(size=.5, width = 0.1, color="black") +
  xlab("") +
  coord_flip() +
...
Value of indicator 2 within a single country, by method to define populations, taxonomic group and iucn.

```{r, out.width="600px", out.height="800px"}

### For loop, 3 plots (defined_populations_ultrasimplified, taxonomic and global IUCN) by country.
...
p1<- x %>% 
  ...
  scale_color_manual(values=scales::hue_pal()(length(levels(as.factor(ind2_data_firstmulti$defined_populations_ultrasimplified)))),
                     breaks=levels(as.factor(ind2_data_firstmulti$defined_populations_ultrasimplified))) +
...
p2<- x %>% 
  ...
  # use ind2_data_firstmulti instead of x to define number of colors and breaks
  # to ensure same colors are used in all countries
  scale_color_manual(values=cividis(length(levels(as.factor(ind2_data_firstmulti$taxonomic_group)))),
...
ind2_data %>%
  ...
  # plot
    ggplot(aes(x=n_extant_populations, y=indicator2, color=defined_populations_ultrasimplified)) +
    geom_point() +
    theme_bw()
...
ggplot(aes(x=country_assessment, fill=taxonomic_group)) +
  geom_bar() +
  scale_fill_manual(values= cividis(length(levels(as.factor(ind3_data$taxonomic_group)))),
