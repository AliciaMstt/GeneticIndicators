...
In total there are `r nrow(ind2_data_multi)` multiassessed records, of `r length(unique(ind2_data_multi$taxon))` taxa. Notice that without filtering missing data in the number of populations, we would have `r sum(ind2_data$multiassessment=="multiassessment")` records of `r nrow(multi_taxa)` taxa.

This is how much the values vary within mutliassessed taxa, excluding missing data:
...
ind2_data_multi %>% 
  ...
  xlab("") +
  coord_flip() +
#  theme(axis.text=element_text(country_assessment)) +
  theme_bw()
