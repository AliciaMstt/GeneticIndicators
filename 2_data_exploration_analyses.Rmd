...
ind2_data %>% 
  ...
  scale_color_manual(values= cividis(length(levels(as.factor(ind2_data$taxonomic_group)))),
...
ind2_data %>% 
  ...
  theme_bw() +
  theme(panel.border = element_blank()) + 
  ggtitle("Distribution of extant populations by taxonomic group and \nmethod to define pops, excluding outliers (>200 pops)") +
  scale_color_manual(values= cividis(length(levels(as.factor(ind2_data$taxonomic_group)))),
                    breaks=levels(levels(as.factor(ind2_data$taxonomic_group)))) +
...
ind2_data %>% 
  filter(n_extant_populations<200) %>%
  ggplot(aes(x=country_assessment, y=n_extant_populations, color=country_assessment)) +
          geom_boxplot() +
  coord_flip() +
  theme_bw() +
  theme(panel.border = element_blank(), legend.position="none") + 
