...
Exclude outliers (>300 populations)
```{r}
ind2_data %>% 
  filter(n_extant_populations<300) %>%
  ggplot(aes(x=n_extant_populations)) +
  geom_density() + 
  ggtitle("Distribution of extant populations \nexcluding outliers (>300 pops)")

```

How does the number of populations vary by country?
...
And by method to define populations?
...
Same plot, but excluding Bombus terricola massive variation:
