...
ggplot(df, aes(x = x,
  ...
  # colour by country
  scale_fill_manual(values=c(scales::hue_pal()(length(unique(ind2_data$country_assessment))),  # a nice  color for each of the n countries
                             rep("darkgrey", length(unique(ind2_data$taxonomic_group)))), # grey for the n taxonomic groups
                    breaks=c(unique(ind2_data$country_assessment), 
                             unique(ind2_data$taxonomic_group))) +
