...
#### Catherine look here: Same m.c3 model but using indicators_full:
```{r}
...
Histogram:
```{r}
ind1_alt_ratios_long %>% ggplot(aes(x=ind1_value, fill=NeNc_ratio)) +
  geom_histogram(alpha=0.5, position = 'identity') + theme_light()
```
Same histogram faceting:
```{r}
ind1_alt_ratios_long %>% ggplot(aes(x=ind1_value, fill=NeNc_ratio)) +
...
Boxplots:
```{r}
ind1_alt_ratios_long %>% ggplot(aes(x=NeNc_ratio, y=ind1_value, fill=NeNc_ratio)) +
  geom_boxplot() + 
  geom_jitter(size=.5, width = 0.1) +
  theme_light()
...
x<-ind1_alt_ratios_long %>% group_by(NeNc_ratio) %>%
                         summarize(n=n(),
                         mean=mean(ind1_value),
                         median=median(ind1_value),
...
sp_known_ratios <-sp_known_ratios %>% mutate(
                    ...
                    ratio_species_related == "<0.1" ~ 0.05,
...
select(sp_known_ratios, ratio_species_related, new_ratio)
```
...
Show nice table
```{r}
x<-select(sp_known_ratios, taxon, taxonomic_group, english_name, country_assessment, ratio_species_related, new_ratio)
kable(x)
```
...
```{r}
summary(sp_known_ratios$new_ratio)
nrow(sp_known_ratios)
table(sp_known_ratios$taxonomic_group)
```

##### Pending:
Use NcPoint data from data_for_alternative_ratio witht he new_ratio for each species to transform the data and calculate the indicator.
