...
### Supplementary Figure S4X: Ne and Nc data availabiltiy by taxa
```{r}
...
## Supplementary Figure S7: Method to define populations used by country and taxa (alluvial)
...
pdf("FigS7.pdf", width = 9, height = 7)
# plot
alluvial(foralluvial[,1:3], freq = foralluvial$n,
         col=methodspop, 
         blocks=FALSE,
...
### Supplementary Figure S8: number of mantained populations by method to define populations and range type
...
ggsave("FigS8.pdf", width = 18, height = 19  , units = "cm")
```
...
### Supplementary Figure S9: Single figure 2 panels scatter plots number of populations vs indicators
...
ggsave("FigS9.pdf", width = 18, height = 28  , units = "cm")
...
ggplot(aes(x=country_assessment, fill=global_IUCN)) +
  ...
  scale_fill_manual(values= IUCNcolors, # iucn color codes
      ...
      theme_light()
```
### Supplementary Figure S10: Sankey plot genetic data availability
...
ggsave("FigS10.pdf", width = 20, height = 18  , units = "cm")
...
#### Supplementary Figure S6: Hist and boxplot with different Ne_Nc ratios

```{r}
plot_grid(hist_ratios + ggtitle(""),
          boxplot_ratios + ggtitle(""), ncol = 1, labels = c("A)", "B)"))

ggsave("FigS6.pdf", width = 18, height = 22  , units = "cm")
```
