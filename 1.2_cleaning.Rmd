...
```{r}
kobo_output<-read.csv(file="International_Genetic_Indicator_testing_V_4.0_-_latest_version_-_False_-_2023-08-21-13-27-33.csv", sep=";", header=TRUE) %>%
...
# run function
get_attached_files(root_dir = "attachments_2023_08_16_14_07_30", # the directory after unziping the kobo attachments download  
...
expected<-kobo_clean %>% 
            #  filter(taxon %!in% to_check$taxon) %>% 
               ...
               select(country_assessment, name_assessor, taxon, X_uuid, map_populations_URL)
...
How many are in No_coincidence? (This likely means that the species assessment was corrected in kobo and the system does not delete the old file. But we can manually check each file if we are unsure):
